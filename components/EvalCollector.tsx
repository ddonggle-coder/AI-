...
interface GradeRow { name: string; pct: string; }

interface EvalCollectorProps {
  onBack: () => void;
}

const EvalCollector: React.FC<EvalCollectorProps> = ({ onBack }) => {
  const [orgName, setOrgName]   = useState('');
  const [gc, setGc]             = useState(5);
  const [grades, setGrades]     = useState<GradeRow[]>(DEFS[5].map(([n,p]) => ({ name:n, pct:p })));
  ...
  const chkExt = (f: File) => EXTS.includes(f.name.split('.').pop()?.toLowerCase() || '');

  const addSFile = (f: File) => {
    if (!chkExt(f)) { alert('지원하지 않는 형식입니다.'); return; }
    setSFiles([f]);
  };
  ...
  const addIFiles = (newFiles: FileList) => {
    setIFiles(prev => {
      ...
      for (let i = 0; i < newFiles.length; i++) {
        const f = newFiles[i];
        if (!chkExt(f))                              { alert(f.name + ': 지원하지 않는 형식'); continue; }
        if (updated.some(ef => ef.name === f.name))  continue;
        if (updated.length >= 20)                    { alert('최대 20개까지 업로드 가능합니다.'); break; }
        updated.push(f);
      }
      return updated;
    });
  ...
  const go = async () => {
    if (!orgName.trim()) { alert('조직명을 입력해주세요.'); return; }
    const gs: string[] = []; const gd: number[] = [];
    for (let i = 0; i < grades.length; i++) {
      const n = grades[i].name.trim();
      const p = parseFloat(grades[i].pct);
      if (!n)      { alert(`${i+1}번째 등급명을 입력해주세요.`); return; }
      if (isNaN(p)){ alert(`${n} 등급의 배분율을 입력해주세요.`); return; }
      gs.push(n); gd.push(p);
    }
    if (!sumOk)  { alert(`배분율 합계가 ${gradeSum}%입니다. 100%로 맞춰주세요.`); return; }
    if (!sType)  { alert('평가자료 유형을 선택해주세요.'); return; }
    const files = sType === 'summary' ? sFiles : iFiles;
    if (!files.length) { alert('파일을 업로드해주세요.'); return; }
    ...
    setLoading(true);
    setLoadingMsg('AI가 평가 데이터를 분석하고 있습니다...\n' + (files.length > 5 ? `파일 ${files.length}개, 2~3분 소요될 수 있습니다.` : '잠시만 기다려주세요.'));
    try {
  ...
  // ─── CSS (원본 HTML 스타일 그대로, 클래스명만 ec- prefix) ───────────────
  const css = `
    ...
    <>
      ...
      {loading && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', zIndex:999,
          ...
          <div style={{ color:'white', marginTop:16, fontSize:14, textAlign:'center',
      ...
      <div className="ec-wrap">
        <div className="ec-card">
          ...
          <div className="ec-bd">
            ...
            {/* ── 조직명 ── */}
            <div className="ec-fg">
              <label>조직명 <span className="ec-req">*</span></label>
              <input
                type="text"
                className="ec-input"
            ...
            {/* ── 등급별 배분 기준 ── */}
            <div className="ec-fg">
              <label>등급별 배분 기준 <span className="ec-req">*</span></label>
              <div className="ec-gc-wrap">
                {[4,5,6].map(n => (
                  <button key={n} className={`ec-gc-btn${gc===n?' active':''}`} onClick={() => changeGc(n)}>
                    {n}단계
                  </button>
                ))}
              </div>
              <table className="ec-gt">
                <thead><tr><th>등급</th><th>배분 기준 (%)</th></tr></thead>
                <tbody>
                  {grades.map((g, i) => (
                    <tr key={i}>
                      <td>
                        <input type="text" className="ec-gi" placeholder="등급명" value={g.name}
                          onChange={e => setGrades(prev => prev.map((r,ri) => ri===i ? {...r, name:e.target.value} : r))} />
                      </td>
                      <td>
                        <input type="text" className="ec-gi" placeholder="%" value={g.pct}
                          onChange={e => setGrades(prev => prev.map((r,ri) => ri===i ? {...r, pct:e.target.value} : r))} />
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="ec-gs" style={{ color: sumOk ? '#059669' : '#DC2626' }}>
                배분 기준 합계: {gradeSum}%
              </div>
              <div className="ec-hint">각 등급명과 목표 배분율(%)을 입력하세요. 합계가 100%가 되어야 합니다.</div>
            </div>

            {/* ── 평가자료 유형 ── */}
            <div className="ec-fg">
              <label>평가자료 유형 <span className="ec-req">*</span></label>
              <div className="ec-type-grid">
                <div className={`ec-tc${sType==='summary'?' selected':''}`} onClick={() => selType('summary')}>
                  <div className="ec-tc-icon">📋</div>
                  <div className="ec-tc-title">등급 총괄표</div>
                  <div className="ec-tc-desc">여러 직원의 등급을<br />한 표에 정리한 파일<br /><b>(1개)</b></div>
                </div>
                <div className={`ec-tc${sType==='individual'?' selected':''}`} onClick={() => selType('individual')}>
                  <div className="ec-tc-icon">📄</div>
                  <div className="ec-tc-title">개인별 평가표</div>
                  <div className="ec-tc-desc">직원 1인당 1파일<br />개인 업적평가표<br /><b>(최대 20개)</b></div>
                </div>
              </div>
            </div>
            ...
            {sType === 'summary' && (
              <div style={{ marginTop:4 }}>
                <div className="ec-uinfo">
                  <b>📋 등급 총괄표</b> — 여러 직원의 등급을 한 표에 정리한 파일 <b>1개</b>를 업로드하세요.<br />
                  ※ 개인별 코멘트가 없는 경우 등급 분포 위주 분석 리포트가 생성됩니다.
                </div>
                <div
                  className={`ec-drop${dropSOver?' over':''}`}
                  onClick={() => fileS.current?.click()}
                  onDragOver={e => { e.preventDefault(); setDropSOver(true); }}
                  onDragLeave={() => setDropSOver(false)}
                  onDrop={e => { e.preventDefault(); setDropSOver(false); if (e.dataTransfer.files[0]) addSFile(e.dataTransfer.files[0]); }}
                >
                  <div className="ec-drop-icon">📋</div>
                  <div className="ec-drop-txt">
                    <b style={{ color:'#3B82F6' }}>클릭</b>하거나 파일을 끌어놓으세요<br />.xlsx .xls .docx .pdf (1개)
                  </div>
                </div>
                <input ref={fileS} type="file" accept=".xlsx,.xls,.docx,.doc,.pdf" style={{ display:'none' }}
                  onChange={e => { if (e.target.files?.[0]) addSFile(e.target.files[0]); e.target.value=''; }} />
                <div className="ec-flist">
                  {sFiles.map((f, i) => (
                    <div key={i} className="ec-fitem">
                      <span>📎 {f.name} ({(f.size/1024).toFixed(1)}KB)</span>
                      <button className="ec-rm" onClick={() => setSFiles([])}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {sType === 'individual' && (
              <div style={{ marginTop:4 }}>
                <div className="ec-uinfo">
                  <b>📄 개인별 평가표</b> — 직원 1인당 1파일로 된 업적평가표를 업로드하세요.<br />
                  평가의견이 포함되어 있으면 더 상세한 코멘트 품질 분석이 가능합니다.<br />
                  ※ 파일 수에 따라 1~3분 소요될 수 있습니다.
                </div>
                <div
                  className={`ec-drop${dropIOver?' over':''}`}
                  onClick={() => fileI.current?.click()}
                  onDragOver={e => { e.preventDefault(); setDropIOver(true); }}
                  onDragLeave={() => setDropIOver(false)}
                  onDrop={e => { e.preventDefault(); setDropIOver(false); addIFiles(e.dataTransfer.files); }}
                >
                  <div className="ec-drop-icon">📄</div>
                  <div className="ec-drop-txt">
                    <b style={{ color:'#3B82F6' }}>클릭</b>하거나 파일을 끌어놓으세요<br />.xlsx .xls .docx .pdf (최대 20개)
                  </div>
                </div>
                <input ref={fileI} type="file" multiple accept=".xlsx,.xls,.docx,.doc,.pdf" style={{ display:'none' }}
                  onChange={e => { if (e.target.files) addIFiles(e.target.files); e.target.value=''; }} />
                <div className="ec-flist">
                  {iFiles.map((f, i) => (
                    <div key={i} className="ec-fitem">
                      <span>📎 {f.name} ({(f.size/1024).toFixed(1)}KB)</span>
                      <button className="ec-rm" onClick={() => setIFiles(prev => prev.filter((_,ri) => ri!==i))}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
